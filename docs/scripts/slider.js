...
let selectedCountryMap = ["Switzerland", "France", "Germany"]

// Extract all dates
const datesRadar = Object.keys(data["France"][places[0]]);
datesRadar.sort();
...
sliderRadar.slider({
    max: datesRadar.length - 1,
    min: 0,
    step: 1,
    slide: (event, ui) => {
        dateIdRadar = ui.value
        dateRadar = datesRadar[dateIdRadar]
        update()
    }
})
...
const step = () => {
    dateIdRadar = dateIdRadar + 1 >= datesRadar.length ? 0 : dateIdRadar + 1
    update()
    sliderRadar.slider('value', dateIdRadar)
}
...
function update() {
    ...
    // Contruct data
    var dataToDraw = []
    for (let country of selectedCountryMap) {
        d = []
        for (let place of places) {
            d.push(
                {
                    axis: placeName[place],
                    value: Math.min(data[country][place][dateRadar] + 1, maxValue)
                }
            )
        }
        dataToDraw.push(d)
    ...
    RadarChart("#radarChart", dataToDraw, selectedCountryMap, dateRadar, radarChartOptions);
}
    ...
    methods: {
        handleSelectionChange() {
            ...
            this.selectedItems.split(',').forEach((c, i) => {
                selectedCountryMap.push(countries[c])
            })
